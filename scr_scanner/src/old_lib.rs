...
pub fn generate(path: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let interfaces = protocol.interfaces.iter().map(|o| {
        ...
        let reqs = o.requests.iter().enumerate().map(|(i, r)| {
            ...
            let opcode = i as u16;
            let (mut return_ty, mut return_stmnt, mut trait_bounds) = (quote! { () }, quote! {}, quote!{});
            ...
            for arg in &r.args {
                ...
                match &arg.arg_type {
                    ...
                    parser::ArgType::NewId { iface } => {
                        ...
                        trait_bounds = quote! {
                            : Dispatch<#iface_mod::#new_type_ob>
                        };
                        new_ids.push(quote!{
                            let #new_idnt = conn.create_new_object(self.version(), #iface_mod::#new_type_ob::INTERFACE, None);
                            // let #new_idnt: #return_ty = Object::from_id(new_id);
                            msg.write_u32(new_idnt.id());
                        });
                        // fn_body.push(quote! {
            ...
            quote! {
                pub fn #req_idnt<S #trait_bounds>(&self, conn: &mut Connection<S>, #(#params,)*) -> #return_ty {
        ...
        quote! {
            ...
            pub mod #iface_mod {
                ...
                impl ::core::cmp::PartialEq<#object_name> for #object_name {
                    fn eq(&self, other: &#object_name) -> bool {
                        self.id == other.id()
                    }
                }
                impl ::core::cmp::Eq for #object_name {}
                ...
                impl WlProxy for #object_name {
                    ...
                    fn from_resources(wl_resource: WlResource) -> Self {
                        Self {
                            id: wl_resource.id(),
                            version: wl_resource.version()
                        }
                    }
                    fn id(&self) -> u32 {
                        self.id
